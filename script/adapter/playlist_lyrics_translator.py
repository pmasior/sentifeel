...
def _get_playlist_translations(playlist):
    cache_handler.make_intermediate_directory("_c_transalations_txt")
    songs_to_analyze = []
    ...
    for song, translation in zip(playlist, songs_with_translations):
        ...
        cache_handler.save_to_intermediate_directory(
            translation_text, "_c_transalations_txt", song["song_id"], "txt"
        )
    return playlist
...
def get_playlists_translations(playlists, playlists_name):
    return cacher.cache2(
        ...
        "_b_playlists_translations_json",
        playlists_name,
        "json",
    )(playlists)
